...
public class ColorPicker : MonoBehaviour
{
    ...
    public static Color[] SpeciesColor = new Color[int.Parse(inputSceneStatus.numTree1)];
    public static Material[] SpeciesMaterial;
    ...
    void Start()
    {
        ...
        showColors = new Texture2D(1, 1, TextureFormat.ARGB32, false);
    ...
    public void _DrawGUI()
    {
        ...
        //Debug.Log(sizeCurr);
        startPos.x = 50 + Screen.width * 2 / 3;
        startPos.y = Screen.height * 2 / 3;
        ...
        GUI.DrawTexture(new Rect(startPos.x + sizeCurr + 10, startPos.y, 40, 20), txColorDisplay);//study
        //GUI.DrawTexture(new Rect(100, 100, 40, 20), showColors);
        //my code
        if (Counter.Count > 1)
        {
            int startX = 50 + Screen.width * 2 / 3;
            int startY = 100;
            int tempX = 0, tempY = 0;
            for (int i = 1; i < (Counter.Count); i++)
            {
                if (showColors != null)
                {
                    showColors.SetPixel(0, 0, SpeciesColor[i - 1]);
                    showColors.Apply();
                }
                GUIStyle labelStyleRGBA = new GUIStyle(GUI.skin.label);
                labelStyleRGBA.normal.textColor = Color.white;
                GUI.Label(new Rect(startX + tempX, startY + tempY, 70, 20), "Species" + i.ToString(), labelStyleRGBA);
                GUI.DrawTexture(new Rect(startX + 80 + tempX, startY + tempY, 40, 20), showColors);
                tempY += 30;
                if (startY + tempY > 400)
                {
                    tempX += 130;
    ...
    {
        SelectedColor = TempColor;
        SpeciesColor[Counter.Count - 1] = SelectedColor;
